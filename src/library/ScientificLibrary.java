...
// класс scientificlibrary реализует интерфейс ilibrary
public class ScientificLibrary implements ILibrary {
    ...
    public IBook[] getBooksSortedByPriceDesc() {
        ...
        IHall bigHall = new ScientificLibraryHall("name", arrSize); // Создание большого зала со всеми книгами всех залов
        int counter = 0;
        for (int i =0; i < halls.length; i++){
            for (int j =0; j < halls[i].getNumberOfBooks(); j++){
